export const filterData = (data, filterBy, value) => {
  if (value === "1000-1500") {
    return data.filter(
      (viaje) => viaje.facts[filterBy] >= 1000 && viaje.facts[filterBy] <= 1500
    );
  } else if (value === "1501-2000") {
    return data.filter(
      (viaje) => viaje.facts[filterBy] >= 1501 && viaje.facts[filterBy] <= 2000
    );
  } else if (value === "2001-2500") {
    return data.filter(
      (viaje) => viaje.facts[filterBy] >= 2001 && viaje.facts[filterBy] <= 2500
    );
  } else if (value === "2501-3000") {
    return data.filter(
      (viaje) => viaje.facts[filterBy] >= 2501 && viaje.facts[filterBy] <= 3000
    );
  }
};
...
export const computeStats = (data) => {
  const sumPrice = data.reduce(
    (accumulator, currentValue) => accumulator + currentValue.facts.cruisePrice,
    0
  );
